...
public class Client {
    ...
    private void parseInput() {
        ...
        try {
            ...
            switch (keyword) {
                ...
                case "groupjoin":
                    ...
                    if (!currentGroup.isEmpty()) {
                        System.out.println("Leaving group '" + currentGroup + "', joining '" + args[0] + "'");
                ...
                case "groupleave":
                    if (groupCheck(args[0])) {
                        System.out.println("You are not in group '" + args[0] + "', you are in " + currentGroup + ".");
                    }
                    currentGroup = "";
                    // TODO: Leave the group
                    break;
            ...
            System.out.println("Invalid Arguments: " + keyword + " takes " + validNumArgsDict.get(keyword) +
                    " arguments, " + args.length + " arguments were provided.\nType 'help' for a list of commands.");
        } catch (Exception e) {
            if (!debugFlag) {
                System.out.println("An unknown error occurred. Type 'help' for a list of commands.");
            }
        }
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        HELP_TABLE.append("-----------------------------------------------------------------------Commands--------------------------------------------------------------------------------\n")
                  ...
                  .append("connect -[address] -[port]               : Connects to the running bulletin board server using the address and port.\n")
                  .append("join                                     : Join the default message board.\n")
                  .append("post -[subject] -[message]               : Posts a message with a subject to the default board.\n")
                  .append("users                                    : Retrieves a list of users in the default group.\n")
                  .append("leave                                    : Leave the default group.\n")
                  .append("message -[id]                            : Retrieve the content of the message given by the id.\n")
                  ...
                  .append("grouppost -[name] -[subject] -[message]  : Posts a message with a subject to the board given by the name.\n")
                  .append("groupusers -[name]                       : Retrieves a list of users in the group given by the name.\n")
                  .append("groupleave -[name]                       : Leaves a specific group.\n")
                  .append("groupmessage -[name] -[id]               : Retrieves the content of a message using its id posted earlier on a message board given by its name in a specific group.\n");
